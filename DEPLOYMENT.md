...
- `SENTRY_DSN` — from your Sentry project (Settings → Client Keys (DSN)). Optional in the sense that the app runs fine without it (`logError` just skips reporting), but every real error captured by `logError` across the app — see `lib/log.ts` — only reaches Sentry once this is set. Not a `NEXT_PUBLIC_` var: this only needs to be read server-side.
...
1. **The SQL judge forks a real child process per query** (`lib/sql/run-isolated.ts`, `fork()` on `lib/sql/sql-worker.cjs`) — deliberate, because only a real OS process can be `SIGKILL`ed reliably to stop a runaway `better-sqlite3` call (`worker_threads` can't; this was verified directly). The fork target is a **runtime-constructed path**, not a static import, so Vercel's function bundler can't discover it by tracing imports and would otherwise silently omit it from the deployed function. **This was confirmed to fail on a real first deploy**: `outputFileTracingIncludes` force-included `sql-worker.cjs` itself, but not what *it* requires — `better-sqlite3` is only ever `require()`d inside that file, which is never part of Next's normal build-tracing graph (only reachable via `fork()` at runtime), so the tracer never followed that require and the native module was missing entirely (`Error: Cannot find module 'better-sqlite3'`, visible in Vercel's Runtime Logs). Fixed by also force-including `better-sqlite3` (native `.node` binary and all) and its own runtime dependency `bindings` in `outputFileTracingIncludes`. If you hit this again after a dependency bump, check Runtime Logs for `runSqlIsolated: child exited unexpectedly` — the `stderr` in that log entry tells you exactly which module is missing.
...
- **Build command**: Vercel's default is just `next build` — this app requires `npx prisma migrate deploy` to run first (see the existing "Deploy steps" section above for why). Override the Build Command in Vercel's Project Settings to: `npx prisma migrate deploy && npx prisma generate && next build`. This means every deploy (including preview deployments, if `DATABASE_URL` is configured for Preview too) applies pending migrations — be deliberate about which Vercel environments get real database credentials.
  - **The explicit `npx prisma generate` matters, not just `npm ci`'s own postinstall hook.** Hit this directly: a schema change (new model, no `package.json` change) shipped, Vercel's dependency cache saw no `package.json`/lockfile change and skipped reinstalling, so `next build` ran against a stale, cached Prisma Client that didn't know about the new model — a real type error at build time (`Property 'x' does not exist on type 'PrismaClient'`), not a flaky one. Fixed two ways at once: `postinstall: "prisma generate"` in `package.json` (so it's not conditional on this specific build command either) and the explicit step here. Belt-and-suspenders on purpose, since a broken production deploy from this is worse than running `generate` twice.
