...
import { getProgrammingSubmissionById } from "@/lib/actions/programming-submissions";

export default async function ProgrammingSubmissionDetailPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  const sub = await getProgrammingSubmissionById(id);
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className={`rounded-xl border px-6 py-4 font-semibold text-lg ${verdictColor}`}>
        ...
        <span className="ml-3 font-normal text-sm">
          {sub.passedTests} / {sub.totalTests} test cases passed
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        ...
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/20 p-4">
          <p className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider mb-1">Language</p>
          <p className="font-body-md text-body-md text-on-surface capitalize">{sub.language}</p>
        </div>
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/20 p-4">
          <p className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider mb-1">Exec Time / Memory</p>
          <p className="font-body-md text-body-md text-on-surface">{sub.executionTimeMs} ms / {(sub.memoryKb / 1024).toFixed(1)} MB</p>
        </div>
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/20 p-4">
